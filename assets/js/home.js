...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!categoryBox || !brandBox) return;
  ...
  try {
    ...
    brandBox.innerHTML = brands.slice(0, 24).map(brand =>
      `<a class="brand-pill" href="/catalog/?brand=${encodeURIComponent(brand)}">${brandIconHtml(brand, 'brand-icon-sm')}<span>${escapeHtml(brand)}</span></a>`
    ).join('');

    installBrandIcons(brandBox);
  } catch (error) {
